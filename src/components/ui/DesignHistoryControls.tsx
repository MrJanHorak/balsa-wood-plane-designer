...
export function DesignHistoryControls({
  ...
  return (
    <div ref={rootRef} className="relative flex items-center gap-1 shrink-0">
      ...
      <button ref={triggerRef} type="button" aria-label={`Named versions, ${versions.length} saved`} aria-expanded={open} aria-haspopup="dialog" onClick={togglePanel}
        ...
        {versions.length > 0 && <span className="rounded bg-slate-700 px-1 text-xs">{versions.length}</span>}
        <ChevronDown className="h-3 w-3 text-slate-400" />
      </button>
      {open && createPortal(
        <div ref={panelRef} role="dialog" aria-label="Named design versions" style={{ ...panelPosition, maxHeight: `calc(100dvh - ${panelPosition.top + 8}px)` }} className="fixed z-[100] w-[min(23rem,calc(100vw-1rem))] overflow-y-auto rounded-xl border border-slate-700 bg-slate-900 shadow-2xl shadow-black/60">
          ...
          <form className="flex gap-2 border-b border-slate-800 p-3" onSubmit={(event) => {
            ...
            <input ref={nameRef} value={name} onChange={(event) => setName(event.target.value)} maxLength={80}
              aria-label="Version name" placeholder="e.g. First flight, wider tail" className="min-h-9 min-w-0 flex-1 rounded-md border border-slate-600 bg-slate-950 px-2.5 py-1.5 text-sm text-slate-100 placeholder:text-slate-400 focus:border-amber-400" />
            ...
            {versions.length === 0 ? (
              ...
              <div key={version.id} className="flex items-center gap-2 rounded-lg px-2 py-2 hover:bg-slate-800/70">
                <div className="min-w-0 flex-1">
                  ...
                  <p className="text-xs text-slate-300" title={version.document.metadata.name}>{version.document.metadata.name} · v{version.document.version} · {new Date(version.createdAt).toLocaleString()}</p>
                </div>
                <button type="button" onClick={() => { onRestoreVersion(version); setOpen(false); }} className="min-h-8 rounded border border-slate-600 px-2 py-1 text-xs text-slate-200 hover:bg-slate-700">Restore</button>
                <button type="button" onClick={() => {
                  if (confirmDelete === version.id) { onDeleteVersion(version.id); setConfirmDelete(null); }
                  else setConfirmDelete(version.id);
                }} className={`min-h-8 min-w-8 rounded p-1.5 text-xs ${confirmDelete === version.id ? 'bg-red-900 text-red-100' : 'text-slate-300 hover:bg-slate-700 hover:text-red-300'}`}
