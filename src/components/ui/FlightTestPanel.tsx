...
export function FlightTestPanel({ tests, aeroReport, nominalDihedralDeg, onAdd, onRemove }: Props) {
  ...
  const numberInput = (key: NumberField, title: string, step = '0.1') => (
    <label className="flex flex-col gap-1 text-[11px] text-slate-300" key={key}>
      {title}
      <input type="number" min="0" step={step} value={numbers[key]} onChange={(event) => setNumber(key, event.target.value)}
        className="w-full rounded-md border border-slate-700 bg-slate-950 px-2 py-1.5 text-xs text-slate-100 focus:border-cyan-400 focus:outline-none" />
    </label>
  );
  ...
  return (
    <section className="rounded-xl border border-slate-800 bg-slate-900/90 shadow-xl">
      <button type="button" onClick={() => setOpen((value) => !value)} aria-expanded={open}
        ...
        {tests.length > 0 && <span className="rounded bg-cyan-950 px-1.5 py-0.5 text-[10px] text-cyan-200">{tests.length}</span>}
          ...
          <form onSubmit={submit} className="space-y-3">
            <label className="flex flex-col gap-1 text-[11px] text-slate-300">
              ...
              <input value={label} maxLength={100} onChange={(event) => setLabel(event.target.value)} placeholder={`Build / flight test ${tests.length + 1}`}
                className="rounded-md border border-slate-700 bg-slate-950 px-2 py-1.5 text-xs text-slate-100 focus:border-cyan-400 focus:outline-none" />
            </label>
            <div>
              <p className="mb-1.5 text-[11px] font-semibold text-slate-200">As built (leave unknown values blank)</p>
            ...
            <div>
              <p className="mb-1.5 text-[11px] font-semibold text-slate-200">Flight</p>
            ...
            <div className="grid grid-cols-2 gap-2">
              ...
              <label className="flex flex-col gap-1 text-[11px] text-slate-300">Actual wing camber
                <select value={wingCamberState} onChange={(event) => setWingCamberState(event.target.value as 'unknown' | 'flat' | 'formed')}
                  className="min-w-0 rounded-md border border-slate-700 bg-slate-950 px-1.5 py-1.5 text-xs">
                  <option value="unknown">Not measured</option><option value="flat">Left flat</option><option value="formed">Formed / curved</option>
                </select>
              </label>
            </div>
            <div className="grid grid-cols-2 gap-2">
              <label className="flex flex-col gap-1 text-[11px] text-slate-300">Pitch
                <select value={pitch} onChange={(event) => setPitch(event.target.value as FlightPitchObservation)} className="min-w-0 rounded-md border border-slate-700 bg-slate-950 px-1.5 py-1.5 text-xs">
                  {Object.entries(PITCH_LABELS).map(([value, text]) => <option value={value} key={value}>{text}</option>)}
                </select>
              </label>
              <label className="flex flex-col gap-1 text-[11px] text-slate-300">Turn
                <select value={turn} onChange={(event) => setTurn(event.target.value as FlightTurnObservation)} className="min-w-0 rounded-md border border-slate-700 bg-slate-950 px-1.5 py-1.5 text-xs">
                  {Object.entries(TURN_LABELS).map(([value, text]) => <option value={value} key={value}>{text}</option>)}
                </select>
              </label>
            </div>
            <label className="flex flex-col gap-1 text-[11px] text-slate-300">Notes or trim changes
              <textarea value={notes} maxLength={2000} rows={2} onChange={(event) => setNotes(event.target.value)} placeholder="What changed between throws? Was the wing formed or left flat?"
                className="resize-y rounded-md border border-slate-700 bg-slate-950 px-2 py-1.5 text-xs focus:border-cyan-400 focus:outline-none" />
            </label>
            ...
            {[...tests].reverse().map((test) => {
              ...
              return <article key={test.id} className="rounded-lg border border-slate-700 bg-slate-950/80 p-3 text-xs text-slate-300 space-y-1">
                <div className="flex items-start gap-2">
                  <div className="min-w-0 flex-1"><p className="font-semibold text-slate-100">{test.label}</p><p className="text-[10px] text-slate-500">{new Date(test.createdAt).toLocaleString()}</p></div>
                  <button type="button" title="Remove test (Undo can restore it)" aria-label={`Remove ${test.label}`} onClick={() => onRemove(test.id)} className="rounded p-1 text-slate-500 hover:bg-slate-800 hover:text-red-300"><Trash2 className="h-3.5 w-3.5" /></button>
                </div>
